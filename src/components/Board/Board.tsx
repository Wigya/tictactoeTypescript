...
function Board({ setScores }: any) {
  ...
  const getAvailableMoves = (board: boardType) => {
  ...
  const handleClick = (id: number) => {
    ...
    if (isTerminal(editedBoard).winner === "X") {
      ...
      setScores((prevState: scoresType) => ({
        ...prevState,
        x: prevState.x + 1,
      }));
      return;
  ...
  const isEmpty = (board: boardType) => {
    return board.every((cell) => !cell);
  };

  const isFull = (board: boardType) => {
    return board.every((cell) => cell);
  };
  ...
  return (
    <div className="board">
      {board.map((val: string, i: number): JSX.Element => {
        return (
          <Square
            key={i}
            id={i}
